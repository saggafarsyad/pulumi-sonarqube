...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `sonarqube` with the `NAME` of your provider.
   - find and replace all instances of the boilerplate `saggafarsyad` with the `ORG` of your provider.
   - replace all instances of the `github.com/saggafarsyad/pulumi-sonarqube` repository with the `REPOSITORY` location
...
before:
  ...
  - make tfgen
builds:
- binary: pulumi-resource-sonarqube
  dir: provider
  goarch:
  - amd64
  - arm64
  ...
  ignore: []
  ldflags:
  - -X github.com/saggafarsyad/pulumi-sonarqube/provider/pkg/version.Version={{.Tag}}
  main: ./cmd/pulumi-resource-sonarqube/
  sort: asc
  use: git
release:
  disable: false
...
├── config.yaml
├── metadata.json
├── pulumi-sonarqube_darwin_amd64_v1
│   └── pulumi-resource-sonarqube
├── pulumi-sonarqube_darwin_arm64
│   └── pulumi-resource-sonarqube
├── pulumi-sonarqube_linux_amd64_v1
│   └── pulumi-resource-sonarqube
├── pulumi-sonarqube_linux_arm64
│   └── pulumi-resource-sonarqube
├── pulumi-sonarqube_windows_amd64_v1
│   └── pulumi-resource-sonarqube.exe
└── pulumi-sonarqube_windows_arm64
    └── pulumi-resource-sonarqube.exe
```
